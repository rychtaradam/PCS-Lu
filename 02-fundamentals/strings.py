...
ceske_datum = '18.10.2020'
casti = ceske_datum.split('.')
ceske_datum = casti[2] + '-' + casti[1] + '-' + casti[0]
print(ceske_datum)

def identifikator(souslovi):
    zmenseni = souslovi.lower()
    propython = zmenseni.replace(" ", "_")
    print(propython)
    prvnipismenovelke = souslovi.title()
    prvnislovo = prvnipismenovelke[0].lower() + prvnipismenovelke[1]
    prvnipismenovelke = prvnipismenovelke.replace(' ', '')
    print(prvnislovo + prvnipismenovelke[2:])

print('Zadejte sousloví: ')
identifikator(input())

print('Zadejte počet osob, pro které chcete vytvořit heslo: ')
osob = input()
y =0

def nahodnePismena():
    pismena = string.ascii_letters
    randomPismena = ''.join(random.choice(pismena) for i in range(0, 3))
    return randomPismena

def nahodneCisla():
    cisla = '0123456789'
    randomCisla = ''.join(random.choice(cisla) for i in range(0, 3))
    return randomCisla

def nahodnyZnak():
    znaky = '@&#{}[]/+-*:()!%'
    randomZnak = ''.join(random.choice(znaky))
    return randomZnak

while y < int(osob):
    zvetseni = nahodnePismena().upper()
    zmenseni = nahodnePismena().lower()
    print(zvetseni + zmenseni + nahodnyZnak() + nahodneCisla())
    y+=1
